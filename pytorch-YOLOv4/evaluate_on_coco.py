...
imagesofinterest = [120263, 280785, 379961]
...
def test(model, annotations, cfg):
    ...
    for i, image_annotation in enumerate(images):
        ...
        if image_id not in imagesofinterest:
            continue
        ...
        start = time.time()
        # order of args do_detect(model, img, conf_thresh, nms_thresh, use_cuda=1):
        boxes = do_detect(model, sized, 0.5, 0.1, use_cuda)
        finish = time.time()
        if type(boxes) == list:
            boxes = boxes[0] # boxes is a list of length 1, need to extract this element first
